...
public class Kniznica {
	...
	public List<Kniha> vypisKnih() {
	    return new ArrayList<>(knihy);
	}
	
	public void setKnihy(List<Kniha> noveKnihy) {
        this.knihy = new ArrayList<>(noveKnihy);
        System.out.println("Zoznam kníh bol aktualizovaný.");
    }
	...
	public void stavKnihy(String nazovKnihy, int stav) {
	    Optional<Kniha> hladanaKniha = knihy.stream()
	                                        .filter(kniha -> kniha.nazov.equals(nazovKnihy))
	                                        .findFirst();
	    if (hladanaKniha.isPresent()) {
	        if (stav == 1) {
	            hladanaKniha.get().vratit();
	        } else {
	            hladanaKniha.get().pozicat();
	        }
	    } else {
